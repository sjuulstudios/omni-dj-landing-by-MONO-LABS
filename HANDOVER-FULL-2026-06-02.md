...
## ✅ SESSIE 69 (2026-06-02) — file-picker-blocker OPGELOST (A + B + C, code-side, dev geverifieerd)

> De upload-blocker hieronder (osascript-picker hangt in de gesignde bundle) is aangepakt
> via ALLE DRIE de voorgestelde opties tegelijk, plus de timeout/foutmelding. Code-side klaar
> en op de source-dev-server (poort 5556) geverifieerd. NOG NIET gecommit/gerebuild.

### Wat is gedaan
- **Optie C — NSOpenPanel (primaire fix), `app.py`.** Nieuw: `_nsopenpanel_supported()`,
  `_pick_with_nsopenpanel(prompt, default_dir, choose_dirs)`. Opent een in-proces Cocoa
  `NSOpenPanel` op de main thread (via `performSelectorOnMainThread_..._waitUntilDone_`).
  GEEN Apple-Event → werkt onder de hardened runtime. `_pick_file_macos` en
  `_pick_folder_macos` proberen dit nu EERST; bij een echte fout loggen ze en vallen terug
  op de oude osascript-route. Geverifieerd: AppKit/objc/Foundation importeren in de venv,
  `_nsopenpanel_supported()` → True.
- **Optie B — apple-events entitlement, `entitlements.plist`.** Toegevoegd:
  `com.apple.security.automation.apple-events`. Houdt de osascript-fallback werkend onder
  hardened runtime. (Bij rebuild wordt dit meegesigned via build_macos.sh.)
- **Optie A — timeout + browser-fallback, `static/index.html`.** `openFilePicker()` racet
  `/api/pick-file` tegen een 7s-timeout. Bij timeout / server-fout / onbereikbaar endpoint
  → toast + val terug op de verborgen `<input type=file>` (→ `uploadFile()` → `/api/upload`,
  omzeilt osascript volledig). Nieuwe helper `_fallbackToBrowserPicker()`. Drag-drop ging al
  via `/api/upload` en is dus al een veilige route.
- **Bundling, `OmniDJ.spec` + `requirements.txt`.** `pyobjc-framework-Cocoa` expliciet
  toegevoegd; hidden-imports `objc/AppKit/Foundation/Cocoa` + `collect_submodules("objc")`
  zodat de PyInstaller-bundle NSOpenPanel echt meeneemt (anders valt de bundle terug op de
  geblokkeerde osascript-route).

### Live geverifieerd (Chrome MCP op source-dev-server :5556, OMNI_DJ_PORT=5556)
- Picker hangt → na ~7s toast "Systeemvenster reageerde niet — kies je bestand hier." +
  browser-input geklikt. GEEN oneindige hang meer.
- Server-fout (`ok:false`) → directe toast + browser-fallback.
- Geldig pad → `_startLocalJob(path)`, geen fallback. Cancel → no-op, geen fallback.
- `app.py` py_compile OK; spec parse OK; nieuwe JS node --check OK.

### ✅ Ook sessie 69 — V1-UI volledig uitgefaseerd, V2 = enige UI (`static/index.html`)
- Doel (Sjuul): users zien V1 niet meer en kunnen niet meer toggelen; alles (login/signup/
  onboarding + app) krijgt standaard de nieuwe oranje V2-UI. V1-code blijft bewaard (bestand +
  git) voor latere referentie.
- 3 kleine wijzigingen: `isV2On()` → `return true` (flag uitgefaseerd); de losse `v2On`-reader
  (~regel 25473) → `true`; de dev-toggle `#v2FlagToggle` verborgen (`hidden`+display:none) en
  de click-handler verwijderd. Geen V1-CSS/DOM gesloopt, geen mount-refactor.
- Oorzaak die dit oploste: de v2-auth-CSS bestond al (oranje, sessie 48/58) maar was gescoped
  onder `body.redesign-v2`; die class kwam alleen op de body als de localStorage-flag aanstond.
  Op een verse install/login (flag null) viel het scherm terug op de oude amber-stijl → wat
  Sjuul op de screenshot zag. Nu staat de class altijd aan.
- Live geverifieerd (Chrome MCP, lege localStorage): `body.redesign-v2` aan, login-knop
  `rgb(217,119,66)`=#D97742, pill-tabs, toggle onzichtbaar; signup-tab toont de oranje
  onboarding-wizard (1/4). Screenshots gemaakt.

### NOG TE DOEN (Sjuul, op de Mac)
1. **git commit + push** — `Omni DJ/app.py`, `Omni DJ/static/index.html`,
   `Omni DJ/entitlements.plist`, `Omni DJ/OmniDJ.spec`, `Omni DJ/requirements.txt`, `HANDOVER.md`.
   (index.html bevat zowel de picker-fix als de V1→V2-uitfasering.)
2. **`pip install` in de venv** zodat de nieuwe dep meekomt in de rebuild:
   cd "/Users/sjuulsmits/Documents/Claude/Projects/Omni DJ/Omni DJ" && venv/bin/pip install "pyobjc-framework-Cocoa>=10.0"
3. **Rebuild gesigned+genotariseerd:** `./build_macos.sh sign notarize dmg`.
4. **Smoke-test in de GESIGNDE bundle:** klik "Choose file" → NSOpenPanel opent (geen hang);
   kies set → analyse start. Drag-drop werkt. Bij picker-fout: nette toast, geen hang.
   DAARNA pas DMG→R2.

> Onderstaande blocker-sectie is de oorspronkelijke diagnose — bewaard voor context.

---

## ⛔ BLOCKER (opgelost in sessie 69) — "kan geen DJ-sets inladen" (file-picker hangt in de bundle)

> Ontdekt na de sessie-68 rebuild. **Diagnose is rond en met bewijs onderbouwd via Chrome MCP
> + computer-use.** Dit is NIET het oude zlib/stale-process-verhaal — dat was los en is opgelost.

### Symptoom
In de **gesignde/genotariseerde bundle** (`/Applications/Omni DJ.app`, bekeken in Chrome op
127.0.0.1:5555) doet "Drop a set here" / "Choose file" niks bruikbaars: er kan geen set worden
ingeladen. De gebruiker blijft op het Analyse-scherm hangen.

### Root cause (bewezen)
- Alle klik-entrypoints (dropzone-klik, "Choose file"-knop, quick-upload) roepen
  `openFilePicker()` aan → die doet `POST /api/pick-file`.
- `/api/pick-file` opent op macOS een native dialog via **`osascript` met `choose file`**
  (`_pick_file_macos`, app.py ~6860).
- In de **hardened runtime** wordt die Apple-Event geblokkeerd: de gesignde binary heeft
  GEEN `com.apple.security.automation.apple-events` entitlement (geverifieerd met
  `codesign -d --entitlements -` → leeg). De Info.plist heeft wél `NSAppleEventsUsageDescription`,
  maar zonder de entitlement laat de hardened runtime de event niet door.
- Gevolg: het dialog verschijnt NERGENS (beide monitoren gecheckt via screenshot) en
  `/api/pick-file` keert nooit terug (>8s geen response in de live test). Daarom "hangt" het laden.
- Verklaart waarom het in DEV (vanuit source, geen hardened runtime) WEL werkt en alleen in de
  gesignde bundle breekt.
- Backend-analyse zelf is OK: directe `/api/upload-local` met een pad verwerkte de testset
  volledig (23 clips, tot cutting) in exact deze bundle. Het probleem zit puur in het
  pad-ophalen via de native picker.

### Fix-opties (kies in volgende sessie)
**Optie A — drag-and-drop als primaire route (kleinste risico, geen Apple-gedoe).**
Drag-drop gebruikt `uploadFile(file)` → `POST /api/upload` (HTTP multipart, bytes-upload) en
omzeilt osascript volledig. Werkt vermoedelijk nu al. Maak drag-drop de hoofdroute en/of laat
de klik terugvallen op de verborgen `<input type=file id="global-file-input">` i.p.v. op
`/api/pick-file`. NB: HTTP-upload heeft wel een praktische groottegrens — voor 10h/100GB-sets
minder ideaal, maar voor normale sets prima. Snelste weg naar "werkt weer".

**Optie B — apple-events entitlement toevoegen (lost de native picker echt op).**
Voeg toe aan `entitlements.plist`:
  `<key>com.apple.security.automation.apple-events</key><true/>`
en herbouw + her-notarizeer. LET OP: dit kan extra Apple-review/permissie-prompts opleveren en
de eerste keer een TCC-toestemmingsdialoog tonen. Test of `choose file` daarna echt opent in de
gesignde build. Combineer eventueel met een timeout in `openFilePicker()` zodat de UI nooit
oneindig hangt + een nette fout toont.

**Optie C — vervang osascript-picker door een echte native panel zonder Apple-Events.**
`choose file` via Standard Additions is fragiel onder hardened runtime. Alternatief: een klein
PyObjC `NSOpenPanel` (geen Apple-Event, draait in-proces) of de tkinter-fallback ook op macOS
toestaan. Meer werk, maar robuust en zonder externe-app-permissies.

**Aanbevolen volgorde:** eerst Optie A (snel terug werkend voor demo/beta), daarna B of C als
je de "grote-bestanden zonder kopie via native picker" wilt behouden. In ELK geval: voeg een
**timeout + zichtbare foutmelding** toe in `openFilePicker()` zodat een falende picker nooit
meer als een stille hang voelt.

### Verificatie-checklist na de fix (in de GESIGNDE bundle, niet dev)
1. Drop een set in "Drop a set here" → analyse start, processing-view verschijnt.
2. Klik "Choose file" → een dialog/-picker opent en levert een pad/bestand → analyse start.
3. Geen oneindige "Bestand registreren…"/hang meer; bij falen een nette toast.
4. Daarna pas DMG→R2 publiceren.

### NIET vergeten
- De sessie-68 DMG is al gebouwd (`dist/Omni DJ.dmg`, Accepted) maar **nog NIET naar R2 gepusht**.
  Publiceer die pas NA deze upload-blocker-fix, anders verspreid je een build waarin een set
  inladen via de knop niet werkt.
