...
# found on disk". Persistent project-relative paths solve that, at the cost
...
HISTORY_PATH      = os.path.join(BASE_DIR, 'job_history.json')
WATCH_FOLDER_PATH = os.path.join(BASE_DIR, 'watch_folder.json')
BRAND_KIT_PATH    = os.path.join(BASE_DIR, 'brand_kit.json')
# SESSIE 21 — Brand Stack: per-asset folders under BASE_DIR/brand_kit/.
...
def _build_export_filename(clip, idx, aspect_key, codec, ext='.mp4'):
    """Compose the user-visible export filename. Prefers the user's custom
    label; falls back to '<Type>_<index>' (e.g. 'Drop_8'). The double-
    underscore separators around aspect/codec/clip-index let the Library
    parser pull each part out reliably even when the user's label happens
    to contain 'landscape' or a number."""
    ...
    # Embed the clip index so thumbnails and "Open in Editor" can find the
    # source clip without re-parsing the user-facing label.
...
def _run_export_job(job_id, cfg):
    ...
    def _process(idx, clip):
        ...
        for aspect_key, src in sources:
            ...
            # Rename the freshly-rendered file to the user-visible label
            # ('My Drop' → 'My_Drop__landscape__match.mp4'). Falls back to
            # '<Type>_<index>' (e.g. 'Drop_8') for un-renamed clips.
            try:
                ext = os.path.splitext(out_path)[1] or '.mp4'
                new_basename = _build_export_filename(clip, idx, aspect_key, codec, ext)
    ...
    for job_id in job_ids:
        ...
        for fn in entries:
            ...
            try:
                ...
                continue
            # Filenames may follow either of two patterns:
            #   New: <label>__clip<NN>__<aspect>__<codec>.mp4
            #   Old: export_<NN>_<jobid>_clip<NN>_<type>_<aspect>_<codec>.mp4
            # Try the unambiguous double-underscore pattern first so we
            # never confuse a digit inside the user's label with the
            # clip index.
            thumbnail = None
            clip_idx = None
            ...
            # Aspect is parsed from the format marker in the filename so the
            # Library can offer 9:16/16:9 filters without re-probing ffmpeg.
            # New filenames use a `__aspect__codec.ext` suffix; legacy names
            # used a single underscore in the middle. Try the new pattern
            # first so user labels containing 'landscape' don't fool us.
            aspect = '16:9'
