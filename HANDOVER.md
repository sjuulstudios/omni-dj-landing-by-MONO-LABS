...
> **MEEST RECENT (2026-06-11, sessie 83):** Quota + payment hardening GEBOUWD EN LIVE: migratie 012 staat op main (na groene wegwerp-branch-audit), update-usage v2 + stripe-webhook v7 gedeployed, en de mid-analysis-upgrade E2E op :5599 is GROEN (alle checks). Zie het sessie-83-blok direct hieronder. NIET gecommit (stapelt op sessie 82, ook nog niet gecommit). Resteert: git commit + DMG-rebuild zodra gewenst (de oude DMG blijft werken via de increment-alias).
>
> **Vorige (sessie 82):** `main` staat op `6facf52` (origin/main, GEPUSHT) en is schoon. SESSIE 82 bouwde C6 Calendar-redesign (frontend-only, alleen `static/index.html`), NIET gecommit, zie het sessie-82-blok. Open DMG-punt blijft: smoke-test van de live DMG door Sjuul (sessie-81-blok). Roadmap-rest na C6: test-infra (pytest + Playwright op de Mac), per-clip inmix-controls, B4 Windows.
>
> **Vorige (sessie 80+81):** Bevat: sessies 77-80 (`ebfbb5f`), B3 Electron-signing (`3bfb0f9`+`4731c8e`), de 1:1/4:5 export-crop-fix (`f5237a6`). De gesignde + genotariseerde DMG met die fix staat LIVE op R2: `https://downloads.omnidj.com/Omni-DJ-1.0.0.dmg` (367.542.120 bytes).
...
**Sessie 83 (2026-06-11) - Quota + payment hardening GEBOUWD + LIVE GEDEPLOYED + E2E-BEWEZEN (plan: `PLAN-QUOTA-PAYMENT-HARDENING-2026-06-11.md`). Fixes A (parallelle starts), B (atomair tellen), C (limieten in 1 tabel), D (downgrade-reset) en F (mid-analysis upgrade). Statisch groen + subagent-review groen (1 testbug gevonden en gefixt, 2 risico's direct meegenomen). NIET gecommit.**

- **LIVE-DEPLOY GEDAAN (zelfde dag, na go van Sjuul):** (1) Wegwerp-branch `sessie83-quota-audit`: VALKUIL: de branch-migratiereplay faalt (`MIGRATIONS_FAILED`, lege schema) omdat de live `profiles`-tabel via het dashboard is gemaakt en niet in de migratie-history zit; opgelost met een minimale profiles-spiegel op de branch (alleen de RPC-relevante kolommen). Daarna 012 toegepast + `tests/test_quota_rpc.sql` ALLE 11 ASSERTIES GROEN (cleanup bereikt = hele blok doorlopen) + RLS-audit nieuwe tabellen GROEN (authenticated ziet alleen eigen reserveringen, kan niets muteren en kan de RPC's niet aanroepen; anon ziet 0 reserveringen, leest wel plan_config; service_role kan de RPC's wel aanroepen). Branch direct verwijderd. (2) Migratie `012_quota_atomic` TOEGEPAST op main (lbabsffxefkrxwzkbzar); security-advisors LEEG; live smoke: 3 plan_config-rijen, RPC antwoordt correct. (3) `update-usage` v2 gedeployed (verify_jwt AAN) + `stripe-webhook` v7 (verify_jwt UIT zoals voorheen, Stripe-signature-verificatie in de functie). (4) E2E `tests/test_mid_analysis_upgrade.py` op :5599 GROEN over de echte stack (dev-server vers herstart met nieuwe app.py): reserve-at-start bewezen (used 1->2 direct bij accept, reservering 'reserved'), webhook-upgrade-simulatie mid-run, analyse maakt af ('done'), GEEN eind-increment (used blijft 0 na de reset), reservering eindigt 'finalised', volgende analyse mag onder Pro. Testaccount `omnidj+test@monohq-labs.com` (wachtwoord = test_quota.sh-default) aangemaakt in Supabase voor dit soort tests en na afloop teruggezet op free/0. Test-excerpt staat in `~/Downloads/omni_e2e_excerpt.mp4` (3 min Lisa Korver, mag weg).

- **Fase 1, migratie `supabase/migrations/012_quota_atomic.sql` (REVIEW ONLY, niet toegepast):** `plan_config` tabel (free=2, pro=10, studio=NULL=onbeperkt; RLS read-only voor anon/authenticated), `quota_reservations` tabel (job_id PK, state reserved/finalised/released; alleen eigen rijen leesbaar), en 3 plpgsql RPC's: `reserve_quota` (FOR UPDATE-lock op profiles + window-roll + limietcheck + reservering in 1 transactie; idempotent per job_id; onbekend plan -> free), `release_quota` (idempotent, floor op 0), `finalise_quota`. RPC's ALLEEN executable door service_role. Toepassen via wegwerp-branch + cross-account RLS-audit, dan main (zelfde flow als 010).
- **Fase 2, `supabase/functions/update-usage/index.ts` HERSCHREVEN:** limits uit `plan_config` (fallback-constante alleen degraded), acties `get` (response-shape ongewijzigd voor oude DMGs) + nieuw `reserve`/`release`/`finalize` (job_id) via de RPC's, en `increment` als compat-alias (reserve onder synthetisch `legacy-<uuid>` id, oude `{ok, used}`-shape). JWT-verificatie ongewijzigd. tsc groen. Backup `.pre-sessie83.bak`.
- **Fase 3, `app.py` + `auth.py`:** `PLAN_LIMITS`-dict WEG -> `_plan_limits()` (plan_config, 5 min cache, fallback `DEFAULT_PLAN_LIMITS`). Nieuw: `_call_quota_rpc`/`_reserve_quota`/`_release_quota`/`_finalise_quota`/`_snap_from_reserve`/`_release_quota_for_job` (dev = RPC via supabase_admin; bundle = edge function met user-JWT; `auth.call_update_usage_edge_function` kreeg `job_id`-param). `/api/upload` + `/api/upload-local`: snapshot-pregate blijft (snelle 402 UX), daarna ATOMAIRE `_reserve_quota` na validatie vlak voor jobstart (afgekeurde upload verbrandt niets; geweigerde reserve ruimt het bestand op); review-R1: alles tussen reserve en thread-start in try/except met release. `_process_job`: start-vangnet reserveert voor watch-folder-jobs (watch_folder.py zelf ONGEMOEID), lege set -> release (zelfde beleid als voorheen: kost geen plek), exception -> release, succes -> ALLEEN `finalise` (geen eind-increment meer = geen race met de Stripe-webhook-reset bij upgrade mid-run; beleid Sectie 4: eenmaal geaccepteerd maakt altijd af, upgrade geldt vanaf de volgende analyse). Review-R2-mitigatie: `_require_job_access` ververst het job-access_token bij elke status-poll zodat release/finalise aan het einde van lange sets in de bundle niet strandt op een verlopen JWT. `_increment_usage` volledig verwijderd.
- **Fase 4, `supabase/functions/stripe-webhook/index.ts`:** beslissing D = RESET bij downgrade. `subscription.deleted` -> plan free + usage 0 + vers window; `subscription.updated` -> idem alleen bij rank-VERLAGING (free=0 pro=1 studio=2). `checkout.session.completed` hield zijn bestaande reset (upgrade = vers tegoed). Backup `.pre-sessie83.bak`.
- **Fase 5, tests (nieuw `tests/`):** `tests/test_quota_rpc.sql` = 11 SQL-asserties voor op een wegwerp-branch (reserve/weiger/idempotent/upgrade-mid-run/release-floor/window-roll/studio-onbeperkt/onbekend-plan/cross-user-guard/eindstaten; print verwacht vs werkelijk, eindigt met "ALLE QUOTA-RPC-TESTS GROEN"). `tests/test_mid_analysis_upgrade.py` + `tests/run_mid_upgrade_test.sh` = E2E op :5599 zonder echte Stripe (webhook gesimuleerd via service_role; vereist OMNI_TEST_EMAIL/PASSWORD/SET).
- **Geverifieerd (sandbox):** py_compile (app/auth/test) groen; tsc beide edge functions groen; pglast SQL + plpgsql parse groen (migratie + SQL-test); logica-harness 8/8 groen op de nieuwe helpers (plan_config-read, cache, fallback, RPC-routing, edge-routing met job_id, faalveilig, snap-normalisatie); subagent-review: geen blokkerende fouten in productiecode, 1 blokkerende testbug (B1, auth-header in status-poll) GEFIXT, R1+R5 GEFIXT, R2 GEMITIGEERD (token-refresh bij poll); innerHTML-audit (issue G): alle userdata-interpolaties gebruiken escapeHtml, geen vondst. 0 em/en-dashes in alle toegevoegde regels.
- **BEKENDE RESTPUNTEN (bewust, niet blokkerend):** (R3) `quota_reservations` groeit onbeperkt (ook `legacy-*` rijen) -> retentie-opruimjob is vervolg; 8-tekens job_id is globale PK, cross-user-botsing is bij beta-schaal verwaarloosbaar en transient (retry krijgt nieuw id). (R4) de window-roll in het edge-'get'-pad is nog niet-atomair (zeldzaam, altijd in voordeel user). (R6) smalle watch-folder race: bestand kan als 'seen' gemarkeerd zijn terwijl het vangnet de reserve weigert -> wordt na window-reset niet automatisch opnieuw opgepakt. Fase 6 (launch checklist) staat nog open: Stripe live keys, generieke error-strings.
- **DEPLOY-VOLGORDE: stappen 1-3 GEDAAN (zie LIVE-DEPLOY-bullet bovenaan dit blok).** Resteert alleen (4): DMG-rebuild + R2-upload wanneer gewenst; de oude DMG in het veld blijft intussen gewoon werken via de increment-alias in update-usage v2.
- **TE COMMITTEN sessie 83 (stapelt op 82):** nieuw `Omni DJ/supabase/migrations/012_quota_atomic.sql`, `Omni DJ/tests/` (3 bestanden); gewijzigd `Omni DJ/app.py`, `Omni DJ/auth.py`, `Omni DJ/supabase/functions/update-usage/index.ts`, `Omni DJ/supabase/functions/stripe-webhook/index.ts`, `HANDOVER.md`. NIET committen: alle `*.pre-sessie83.bak`.

Detail: memory `project_sessie83_quota_hardening`.

---

**Sessie 82 (2026-06-09) - C6 Calendar-redesign GEBOUWD (frontend-only, alleen `static/index.html`; backend, `/api/calendar/*`, CRUD-flow en localStorage-terugval ONGEMOEID). Zelfde recept als C5 (sessie 79). Statisch + logica groen, NIET visueel/login-getest (de view zit achter login), NIET gecommit. Stapelt op een schone main (`6facf52`); enige niet-gecommitte wijziging = `static/index.html` + `HANDOVER.md`.**

- **Context:** Sjuul koos C6 uit de roadmap en koos de VOLLEDIGE scope. Diagnose: de Calendar-functionaliteit was al compleet (maand/week-view, schedule-modal, volledige CRUD naar Supabase per workspace sinds sessie 74), maar de copy was deels Nederlands, de "Preview"-banner was stale (zei "blijft lokaal in je browser" terwijl ingelogd echt in Supabase opslaat), en er was geen overzicht/rail.
- **Gebouwd (alleen `static/index.html`):** (1) Nieuwe layout `.cal-layout` = grid-hoofdcanvas (`.cal-main`) links + rechter-rail (`#cal-rail`) rechts. (2) Rail-renderer `_v2RenderRail`: "Up next" (eerstvolgende 6 posts vanaf vandaag, klikbaar -> calOpenEdit) + "Posts by platform" (telling per platform met kleurstip, dubbelt als legenda; toont altijd alle 4 met 0+). (3) Derde view-modus "Agenda" (`data-cal-mode="agenda"`, `#cal-agenda-wrap`) via `_v2RenderAgenda`: chronologische lijst gegroepeerd per dag, alleen upcoming. (4) Helpers `_v2FlatEvents`/`_v2UpcomingEvents`/`_v2EventName` + `_V2_PLAT_META` (platform-naam/kleur). (5) `renderCalendar` dispatcht nu 3 modi + roept de rail aan; label toont "Upcoming" in agenda-modus; `mode`-default blijft 'month'. (6) ALLE user-facing copy naar Engels: header-sub, banner (nu kloppend voor in/uitgelogd), de hele schedule-modal (sub/label/placeholder/Save/Cancel/Delete) en de 9 toasts in calSaveSchedule/calDeleteSchedule. (7) CSS-blok (SESSIE 82-marker) voor rail/agenda/responsive (rail klapt onder de grid < 1100px). De mode-switch-delegatie (`[data-cal-mode]`) pakt de Agenda-knop automatisch.
- **Geverifieerd (sandbox, statisch + logica):** `node --check` op de inline SPA-JS groen; nieuwe id's (cal-agenda-wrap/cal-agenda/cal-rail) elk 1x; nieuwe functies 1x gedefinieerd; `#view-calendar` tag-balans 15/15 div + 1/1 aside; 0 em/en-dashes + 0 dash-entities in de 271 toegevoegde regels (diff vs backup). Node-harness met nep-DOM bewees de renderers: flatten/sort correct, upcoming-filter sluit verleden uit, agenda-groepering + platform-tellingen kloppen, lege-staten werken, geen exceptions. Backup `static/index.html.pre-sessie82-c6.bak`.
- **NIET getest (vereist login op :5599, voor Sjuul; frontend-only dus GEEN server-restart):** reload `http://127.0.0.1:5599/?v=s82`, log in, ga naar Calendar. (1) Rechts een rail met "Up next" + "Posts by platform". (2) Klik "Agenda" -> chronologische lijst per dag van komende posts. (3) Schedule a post -> modal volledig Engels -> opslaan toont "Draft saved" + verschijnt in grid, rail en agenda. (4) Klik een post in de rail/agenda -> opent in edit-modus. (5) Smal venster (< 1100px) -> rail klapt onder de grid. (6) No-regression: maand/week-view + analyse/export werken nog.
- **TE COMMITTEN sessie 82:** gewijzigd `Omni DJ/static/index.html` (C6 Calendar-redesign) + `HANDOVER.md`. NIET committen: `static/index.html.pre-sessie82-c6.bak`.

Detail: memory `project_sessie82_c6_calendar_redesign`.
...
- **GEPUSHT (2026-06-09):** `git push origin main` GEDAAN -> `16e79fd..6facf52 main -> main` op `sjuulstudios/omni-dj-landing-by-MONO-LABS`. Alle 5 commits live op origin/main: `ebfbb5f` (77-80) + `3bfb0f9` + `4731c8e` (B3) + `f5237a6` (1x1-fix) + `6facf52` (HANDOVER). Werkboom heeft nog ongetrackte temp-scripts (`_verify_1x1_fix.sh`, `electron/_build_sessie8*.sh`) + een ongestagede `system_fonts_cache.json` (machine-specifiek) - bewust niet meegepusht.
- **ENIGE NOG OPEN:** smoke-test van de LIVE DMG door Sjuul (download van omnidj.com, openen, picker/login/analyse/export, en bevestig dat 1:1 nu een echte gevulde crop is met de DJ gecentreerd).
